...
enum Command {
    #[command(description = "hace Ping Pong!")]
    Ping,
...
fn make_keyboard() -> InlineKeyboardMarkup {
    ...
    for botones in botones.chunks(3) {
        let row : Vec<InlineKeyboardButton> = botones
            .iter()
    ...
    for versions in comandos_zip.chunks(3) {
        let row = versions
            .iter()
            .map(|&version| InlineKeyboardButton::callback(version.to_owned(), version.to_owned()))
            .collect();
...
async fn inline_query_handler(
    ...
    let choose_debian_version = InlineQueryResultArticle::new(
        ...
        InputMessageContent::Text(InputMessageContentText::new("Elige un Botón:")),
    )
